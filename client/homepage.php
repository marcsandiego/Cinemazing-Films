...
require 'dbconfig.php';
 ...
 <title>Cinemazing Films | Home </title>
  ...
  <?php 
    ...
    $query_e = "SELECT * FROM customer_accounts WHERE username = '$username'";
    $query_run = mysqli_query($connect, $query_e);
    $getData = mysqli_num_rows($query_run);

    if($getData){
      while($row = mysqli_fetch_array($query_run)){
        $_SESSION['email'] = $row['email'];
        $_SESSION['c_coins'] = $row['c_coins'];
  ...
  <div class="middle py-5">
    <div class="container py-lg-5 py-md-4 py-3">
      <div class="welcome-left text-center" data-aos="zoom-out">
        ...
        <a href="add_ccoins.php" class="btn btn-style btn-border mt-sm-5 mt-4">Cinema Coins</a>
      </div>
    </div>
  </div>
  <!-- //quote -->
